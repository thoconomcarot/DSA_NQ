...
//goi ham dao ngược số và cách làm phép trừ như phép trừ tay 
BigInteger subtract(BigInteger a, BigInteger b) {
    BigInteger result;
    ...
    for (int i = 0; i < maxLength; i++) {
        ...
        //nếu mà trừ nhau ra số âm thì mượn 10 (+10) để trừ tiếp 
        if (diff < 0) {
            diff += 10;
            borrow = 1;
        //nếu trừ nhau = dương thì mượn = 0
        } else {
            borrow = 0;
        }
        //trả về kết quả của số nguyên
