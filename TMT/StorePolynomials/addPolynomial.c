...
int empty(Polynomial L){
    return L.size == 0;
}

int fullList(Polynomial L){
...
Polynomial add(Polynomial a1, Polynomial a2, Polynomial *sum){
    ...
    int exp1 = a1.exponent, exp2 = a2.exponentl
    if(exp1 >= exp2){
        return exp1;
    }
    else{
...
//             sum->mono[sum->size].coefficient = a1.mono[p].coefficient + a2.mono[q].coefficient;
